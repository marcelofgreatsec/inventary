...
import {
    Plus, Trash2, Server, Loader2, X,
    Search, Filter, Copy, Check, Edit2,
    Wifi, WifiOff, AlertTriangle, Activity
} from 'lucide-react';

const STATUS_MAP: Record<string, string> = {
    'Online': 'badge-green', 'Offline': 'badge-red', 'Manutenção': 'badge-amber'
};
...
function InfraModal({ onClose, onSave, infra }: { onClose: () => void; onSave: () => void; infra?: Infra }) {
    const [form, setForm] = useState(infra ? {
        name: infra.name, type: infra.type, status: infra.status,
        ip: infra.ip || '', os: infra.os || '', cpu: infra.cpu || '',
        ram_gb: infra.ram_gb?.toString() || '', disk_gb: infra.disk_gb?.toString() || '',
        ...
        <div className="modal-overlay">
            <div className="modal" style={{ maxWidth: 580 }} ref={modalRef} role="dialog" aria-modal="true" aria-labelledby="infra-modal-title">
                <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', marginBottom: 22 }}>
                    <h2 id="infra-modal-title" className="modal-title" style={{ marginBottom: 0 }}>
                        {infra ? 'Editar Equipamento' : 'Novo Equipamento'}
                    </h2>
                ...
                <form onSubmit={handleSubmit}>
                    <div className="form-group">
                        <label>Nome *</label>
                        <input className="input" value={form.name} onChange={f('name')} required placeholder="SRV-APP-01" />
                    </div>
                    ...
                    <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 12 }}>
                        <div className="form-group">
                            ...
                            <input className="input" value={form.ip} onChange={f('ip')} placeholder="10.0.0.1" style={{ fontFamily: 'JetBrains Mono' }} />
                        </div>
                        <div className="form-group">
                            <label>Sistema Operacional</label>
                            <input className="input" value={form.os} onChange={f('os')} placeholder="Ubuntu 22.04 LTS" />
                        </div>
                    </div>
                    <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr 1fr', gap: 12 }}>
                        <div className="form-group">
                            <label>CPU</label>
                            <input className="input" value={form.cpu} onChange={f('cpu')} placeholder="8 vCPUs" />
                        </div>
                        <div className="form-group">
                            <label>RAM (GB)</label>
                            <input className="input" type="number" value={form.ram_gb} onChange={f('ram_gb')} placeholder="32" />
                        </div>
                        <div className="form-group">
                            <label>Disco (GB)</label>
                            <input className="input" type="number" value={form.disk_gb} onChange={f('disk_gb')} placeholder="500" />
                        </div>
                    </div>
                    <div className="modal-footer">
                        ...
                        <button type="submit" className="btn btn-primary" disabled={saving}>
                            {saving
                                ...
                                : infra ? 'Atualizar' : 'Salvar Equipamento'
                            }
                        </button>
                    </div>
                </form>
...
function CopyBtn({ text }: { text: string }) {
    ...
    return (
        <button
            ...
            title="Copiar IP"
            style={{ background: 'none', border: 'none', color: copied ? 'var(--green)' : 'var(--text-muted)', cursor: 'pointer', display: 'flex', alignItems: 'center', transition: 'color 0.2s' }}
        >
...
export default function InfraPage() {
    ...
    const filtered = useMemo(() => infra.filter(i => {
    ...
    const manut    = infra.filter(i => i.status === 'Manutenção').length;
    ...
    const statusCards = [
        { label: 'Online',     value: online,  icon: Wifi,          color: 'var(--green)',  badge: 'badge-green' },
        { label: 'Offline',    value: offline, icon: WifiOff,       color: 'var(--red)',    badge: 'badge-red'   },
        { label: 'Manutenção', value: manut,   icon: AlertTriangle, color: 'var(--amber)',  badge: 'badge-amber' },
        { label: 'Total',      value: infra.length, icon: Activity, color: 'var(--blue)',   badge: 'badge-blue'  },
    ];
        ...
        <div>
            ...
            {(statusFilter !== 'Todos' || debouncedSearch) && (
                <div style={{ display: 'flex', alignItems: 'center', gap: 8, marginBottom: 14 }}>
                    <span style={{ fontSize: 11, color: 'var(--text-muted)', fontFamily: 'JetBrains Mono' }}>Filtros:</span>
                    {statusFilter !== 'Todos' && (
                        <span style={{ display: 'inline-flex', alignItems: 'center', gap: 6, padding: '3px 10px', background: 'var(--blue-dim)', border: '1px solid var(--blue-mid)', borderRadius: 20, fontSize: 11, color: 'var(--blue)' }}>
                            {statusFilter}
                            <button onClick={() => setStatusFilter('Todos')} aria-label="Remover filtro de status" style={{ background: 'none', border: 'none', cursor: 'pointer', color: 'inherit', padding: 0, lineHeight: 1, fontSize: 14 }}>×</button>
                        </span>
                    )}
                    {debouncedSearch && (
                        <span style={{ display: 'inline-flex', alignItems: 'center', gap: 6, padding: '3px 10px', background: 'var(--blue-dim)', border: '1px solid var(--blue-mid)', borderRadius: 20, fontSize: 11, color: 'var(--blue)' }}>
                            "{debouncedSearch}"
                            <button onClick={() => setSearch('')} aria-label="Limpar busca" style={{ background: 'none', border: 'none', cursor: 'pointer', color: 'inherit', padding: 0, lineHeight: 1, fontSize: 14 }}>×</button>
                        </span>
                    )}
                </div>
            )}
                    ...
                    <div className="empty">
                        ...
                        {infra.length === 0 ? (
                            <>
                                ...
                                <button className="btn btn-primary" style={{ marginTop: 16 }} onClick={() => { setEditingInfra(undefined); setModal(true); }}>
                                    <Plus size={15} /> Adicionar primeiro equipamento
                                </button>
                            </>
                        ) : (
                            <>
                                    ...
                                    <tr key={i.id}>
                                        ...
                                        <td>
                                            <div style={{ display: 'flex', alignItems: 'center', gap: 6, fontFamily: 'JetBrains Mono', fontSize: 12, color: 'var(--text-secondary)' }}>
                                                {i.ip || <span style={{ color: 'var(--text-muted)' }}>—</span>}
                                                {i.ip && <CopyBtn text={i.ip} />}
                                            </div>
                                        </td>
                                        <td style={{ fontSize: 12, color: 'var(--text-secondary)' }}>{i.os || <span style={{ color: 'var(--text-muted)' }}>—</span>}</td>
                                        <td style={{ fontSize: 12, fontFamily: 'JetBrains Mono' }}>{i.cpu || <span style={{ color: 'var(--text-muted)' }}>—</span>}</td>
                                        <td style={{ fontSize: 12, fontFamily: 'JetBrains Mono' }}>{i.ram_gb ? `${i.ram_gb} GB` : <span style={{ color: 'var(--text-muted)' }}>—</span>}</td>
                                        <td style={{ fontSize: 12, fontFamily: 'JetBrains Mono' }}>{i.disk_gb ? `${i.disk_gb} GB` : <span style={{ color: 'var(--text-muted)' }}>—</span>}</td>
                                        <td>

...
function LicenseModal({ onClose, onSave }: { onClose: () => void; onSave: () => void }) {
    ...
    const handleSubmit = async (e: React.FormEvent) => {
        ...
        await fetch('/api/licenses', { method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({ ...form, seats: parseInt(form.seats), monthly_cost: parseFloat(form.monthly_cost), renewal_date: form.renewal_date || null }) });
            ...
            <div className="modal">
                ...
                <form onSubmit={handleSubmit}>
                    ...
                    <div className="modal-footer">
                        ...
                        <button type="submit" className="btn btn-primary" disabled={saving}>{saving ? <Loader2 size={15} style={{ animation: 'spin 0.7s linear infinite' }} /> : 'Salvar Licença'}</button>
                    </div>
                </form>
            </div>
        </div>
                            ...
                            <tbody>
                                {licenses.map(l => (
                                    <tr key={l.id}>
                                        ...
                                        <td><button onClick={() => handleDelete(l.id)} className="btn btn-danger" style={{ padding: '5px 10px' }}><Trash2 size={14} /></button></td>
                                    </tr>
                                ))}
                            </tbody>
                        </table>
